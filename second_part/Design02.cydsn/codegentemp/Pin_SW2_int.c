...
{
    CyRamVectors[CYINT_IRQ_BASE + Pin_SW2_int__INTC_NUMBER] = address;
}
...
{
    return CyRamVectors[CYINT_IRQ_BASE + Pin_SW2_int__INTC_NUMBER];
}
...
void Pin_SW2_int_SetPriority(uint8 priority)
{
    ...
    uint32 priorityOffset = ((Pin_SW2_int__INTC_NUMBER % 4u) * 8u) + 6u;
    
	interruptState = CyEnterCriticalSection();
    *Pin_SW2_int_INTC_PRIOR = (*Pin_SW2_int_INTC_PRIOR & (uint32)(~Pin_SW2_int__INTC_PRIOR_MASK)) |
...
uint8 Pin_SW2_int_GetPriority(void)
{
    ...
    priority = (*Pin_SW2_int_INTC_PRIOR & Pin_SW2_int__INTC_PRIOR_MASK) >> priorityOffset;
